...
function ComplianceCheck2() {
  ...
  const [locationDesc] = useState("Behind building line");
  const [result, setResult] = useState(null);
  const [rule, setRule] = useState(null);

  // Load dev types + zones + JSON data
  useEffect(() => {
    ...
    fetch("/random_structures.json")
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch JSON: " + res.status);
        return res.json();
      })
      .then((data) => {
        const lengths = [...new Set(data.map((d) => d.length_m))];
        const widths = [...new Set(data.map((d) => d.width_m))];
        const heights = [...new Set(data.map((d) => d.height_m))];
        const setbacks = [...new Set(data.map((d) => d.setback_m))];

        setDimensionOptions({ lengths, widths, heights, setbacks });

        // Pre-fill with first record
        if (data.length > 0) {
          setLength(data[0].length_m);
          setWidth(data[0].width_m);
          setHeight(data[0].height_m);
          setSetback(data[0].setback_m);
        }
      })
      .catch((err) => {
        console.error("Error loading random_structures.json:", err);
      });
  }, []);
  ...
  const checkCompliance = () => {
    ...
    axios
      .post("http://127.0.0.1:8000/check-compliance1", payload)
      ...
      <div className="form-box">
        ...
        <div className="grid">
          <div>
            ...
            >
              {dimensionOptions.lengths.map((val, i) => (
          ...
          <div>
            ...
            >
              {dimensionOptions.widths.map((val, i) => (
                <option key={i} value={val}>
                  {val}
                </option>
              ))}
          ...
          <div>
            ...
            >
              {dimensionOptions.heights.map((val, i) => (
                <option key={i} value={val}>
                  {val}
                </option>
              ))}
          ...
          <div>
            ...
            >
              {dimensionOptions.setbacks.map((val, i) => (
                <option key={i} value={val}>
                  {val}
                </option>
              ))}
      ...
      <div className="info-box">
        <div className="result1 instructions-box">
          ...
          <ul>
            ...
            <li>2. Choose length, width, height, and setback from dropdowns.</li>
            <li>3. Floor area is automatically calculated.</li>
            <li>4. Click "Check Compliance" to view results.</li>
          </ul>
        </div>
        ...
        {result && (
          <div className={`results ${result.compliant ? "success" : "error"}`}>
            {result.compliant ? <h3>Compliant</h3> : <h3>Non-Compliant</h3>}
...
export default ComplianceCheck2;
